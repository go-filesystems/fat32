#!/usr/bin/env bash
#
# compare.sh — reference (in-kernel vfat/FAT32 + coreutils) throughput for the
# same standard operations the Go benchmarks (bench_test.go) measure, so the
...
#
# Caveat: the kernel vfat driver has the page cache and writeback; the Go
...
IMG=${IMG:-/tmp/fat32-ref.img}
MNT=${MNT:-/tmp/fat32-ref-mnt}
SIZE_MB=${SIZE_MB:-64}
BIG_MB=${BIG_MB:-8}
NFILES=${NFILES:-150}
...
echo "== reference: in-kernel vfat/FAT32 (dosfstools + coreutils) =="
echo "image=${SIZE_MB}MiB bigfile=${BIG_MB}MiB nfiles=${NFILES}"
echo

# --- Format (mkfs.vfat -F 32) ---
rm -f "$IMG"; truncate -s "${SIZE_MB}M" "$IMG"
t0=$(ms); mkfs.vfat -F 32 "$IMG" >/dev/null 2>&1; t1=$(ms)
report "Format (mkfs.vfat)" "$((t1 - t0)) ms"
