// Command bench measures go-filesystems/fat32 Format and read throughput.
//
...
func doFormat(args []string) {
	...
	_ = os.Remove(img)
	start := time.Now()
	fs, err := fat32.Format(img, size, fat32.FormatConfig{Label: "bench"})
	if err != nil {
		fail("Format: %v", err)
	}
